...
uses
  ...
  XMLIntf, Vcl.ComCtrls,
  uClasses, Vcl.Buttons, ICSSpinLabeledEdit,
...
begin
  ...
  try
    if cbProtocol.ItemIndex >= 0 then begin
      ...
      for var I := 0 to iSoftwareNode.ChildNodes.Count - 1 do begin
        for var J := 0 to iSoftwareNode.ChildNodes[I].ChildNodes[ND_PROTOCOLS].ChildNodes.Count - 1 do begin
      ...
      for var I := 0 to cbSoftware.ItemsEx.Count - 1 do if xmlGetItemString(IXMLNode(cbSoftware.ItemsEx[I].Data), ND_PARAM_ID) = xmlGetItemString(FXMLNode.ChildNodes[ND_SOFT_ID], ND_PARAM_VALUE) then begin
        cbSoftware.ItemIndex := I;
        Break;
      end;
      if (cbSoftware.ItemIndex < 0) and (cbSoftware.ItemsEx.Count > 0) then cbSoftware.ItemIndex := 0;
    end;
  finally
    Ico.Free;
    SetControlEnables;
...
begin
  ...
  cbUnsetModifies.State := TCheckBoxState(xmlGetItemInteger(FXMLNode.ChildNodes[ND_UNSET_MODIFIES], ND_PARAM_VALUE));

  MemoStrings.Lines.Text := icsB64Decode(xmlGetItemString(FXMLNode.ChildNodes[ND_PARAMSTRINGS1], ND_PARAM_VALUE));
...
begin
  ...
  xmlSetItemInteger(FXMLNode.ChildNodes[ND_UNSET_MODIFIES], ND_PARAM_VALUE, Ord(cbUnsetModifies.State));

  xmlSetItemString(FXMLNode.ChildNodes[ND_PARAMSTRINGS1], ND_PARAM_VALUE, icsB64Encode(MemoStrings.Lines.Text));
...
begin
  ...
  cbAlerts.Enabled := FMultiEdit or cbUseAlerts.Checked;
  cbUnsetModifies.Enabled := FMultiEdit or cbCheckModified.Checked;
end;
...
begin
  inherited;
  xmlSetItemInteger(FXMLNode, ND_PARAM_LAST_PAGE_IDX, PageControlMain.TabIndex);
end;
...
begin
  inherited;
  PageControlMain.ActivePageIndex := xmlGetItemInteger(FXMLNode, ND_PARAM_LAST_PAGE_IDX);
  SetControlEnables;
end;
...
begin
  ...
  if iNode.ChildNodes.Count = 0 then begin
    ...
    MI.Enabled := False;
    MI.Caption := ICSLanguagesBase.CurrentStrings[34];
    MI.ImageIndex := 0;
